...
| --------------------- | ---------------- | ---------------------------------------------------------------- | ------------------------------------------- |
| Global variables      | aws_region       | Region to deploy in.                                             |                                             |
|                       | vpc_endpoints    | List of centralised VPC endpoints to be deployed                 |                                             |
| Environment variables | ipam_cidr        | CIDR to be allocated to the IP Address Manager                   |                                             |
...
| env_config | Object for environment configuration | <pre>object({<br>    ipam_cidr        = string<br>    tgw_route_tables = list(string)<br>    root_domain      = string<br>  })</pre> | n/a | yes |
